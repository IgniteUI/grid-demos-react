const DATA_URL =
  'https://www.infragistics.com/grid-examples-data/data/finance/finance.json';
...
class DataService {
  public getFinanceData = async () => {
    try {
      ...
      currData.forEach((record: any) => {
        ...
        record['marketValue'] = parseFloat(
          (record.value.currentPrice * record.positions).toFixed(2)
        );
  ...
  private calculateDailyPercentageChange = (
    initialPrice: number,
    finalPrice: number
  ) => {
  ...
  private randomizeData(dataRow: any): {
    ...
    const newMarketValue = parseFloat(
      (newPrice * dataRow.positions).toFixed(2)
    );
    const newDailyPercentage = this.calculateDailyPercentageChange(
      dataRow.initialPrice,
      newPrice
    );
    return {
      newPrice,
